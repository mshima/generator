...
class Conflicter {
  constructor(adapter, force, options = {}) {
    ...
    this.fs = options.destinationFs || fs;
  ...
  _printDiff(file) {
    ...
    if (file.binary) {
      this.adapter.log.writeln(
        binaryDiff.diff(file.path, file.contents, this.destinationFs)
      );
    } else {
      const existing = this.fs.readFileSync(file.path);
      this.adapter.diff(
        existing.toString(),
  ...
  _detectConflict(file) {
    ...
    if (this.fs.statSync(filepath).isDirectory()) return true;

    if (file.binary === undefined) {
      file.binary = binaryDiff.isBinary(file.path, file.contents);
    }

    const actual = this.fs.readFileSync(path.resolve(filepath));
  ...
  _ask(file, cb, counter) {
    ...
    if (this.fs.statSync(file.path).isFile()) {
      prompt.choices.push({
        key: 'd',
