...
class Generator extends EventEmitter {
  ...
  constructor(args, options) {
    ...
    this.destinationFs = this.options.destinationFs || this.env.destinationFs || fs;
    ...
    this.conflicter = new Conflicter(this.env.adapter, this.options.force, {
      ...
      destinationFs: this.destinationFs
    });
  ...
  destinationRoot(rootPath, skipEnvironment = false) {
    if (typeof rootPath === 'string') {
      ...
      if (!this.destinationFs.existsSync(this._destinationRoot)) {
        if (semver.gte(process.version, '10.12.0') || this.destinationFs !== fs) {
          this.destinationFs.mkdirSync(this._destinationRoot, { recursive: true });
        } else {
          // Remove for generator 5
