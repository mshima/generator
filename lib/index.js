...
class Generator extends EventEmitter {
  ...
  constructor(args, options) {
    ...
    if (!this.env) {
      throw new Error(
        `This generator (${this.options.namespace}) requires a Environment to run.`
      );
    }

    ['adapter', 'runLoop', 'sharedFs', 'emit', 'fs'].forEach(requiredField => {
      if (this.env[requiredField]) {
        return;
      }

      throw new Error(
        "Current environment doesn't provides some necessary feature this generator requires"
      );
    });
